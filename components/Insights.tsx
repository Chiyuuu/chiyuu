  ...
  if (logs.length < 1) {
    return (
        <div className="text-center p-8 flex flex-col items-center h-full justify-center">
            ...
            <h2 className="text-xl font-bold text-brand-text">您的专属洞察</h2>
            <p className="text-brand-text-light mt-2 max-w-xs">开始记录，发现您的专属行为模式。</p>
        </div>
    )
  }
    ...
    <div className="p-4 md:p-6 space-y-6">
      <h1 className="text-3xl font-bold text-brand-text">您的个人模式</h1>
       ...
       <InsightCard 
        ...
        {analysis.trigger ? (
            <div className="flex items-center">
                ...
                <p>我们发现，当您感到 <span className="font-bold">“{analysis.trigger}”</span> 时，常常会与进食行为产生关联。</p>
            </div>
            ) : <p>数据还不够多，继续记录，我们一起发现您的主要诱因！</p>}
      </InsightCard>
       ...
       <InsightCard 
        ...
        >
        {analysis.riskTime ? (
            <p>您似乎更可能在 <span className="font-bold">{analysis.riskTime.day}{analysis.riskTime.time}</span> 经历情绪性进食。</p>
        ) : <p>我们仍在学习您的每周节律，更多记录将帮助我们一起找到那个特别的时段。</p>}
      </InsightCard>
